...
class BluetoothConnection {
  ...
  BluetoothConnection._consumeConnectionID(int? id,dynamic uuids)
      : this._id = id,
        this.uuids = uuids,
        this._readChannel =
    ...
    var validRUUID = getValidRUUID();
    var validWUUID = getValidWUUID();
    output.r_service_uuid = validRUUID["service_uuid"];
    output.r_character_uuid = validRUUID["character_uuid"];
    output.w_service_uuid = validWUUID["service_uuid"];
    output.w_character_uuid = validWUUID["character_uuid"];
  }
  ...
  getValidRUUID(){
    if(uuids != null){
      for (int i = 0; i < uuids.length; i++) {
        var item = uuids[i];
        if(item is Map){
          var service_uuid = item['service_uuid'];
          var sb = service_uuid.toString().startsWith("6e");
          if(service_uuid != null && sb){
            var character_uuid_list = item['character_uuid'];
            if(character_uuid_list is List && character_uuid_list.length > 0){
              for (var value in character_uuid_list) {
                var cb = value.toString().startsWith("6e400003");
                if(value != null && cb){
                  return {
                    "service_uuid":service_uuid,
                    "character_uuid":value,
                  };
                }
              }
            }
      ...
      for (int i = 0; i < uuids.length; i++) {
        ...
        if(item is Map){
          ...
          if(service_uuid != null && sb){
            ...
            if(character_uuid_list is List && character_uuid_list.length > 0){
              for (var value in character_uuid_list) {
                var cb = value.toString().startsWith("6e400002");
                if(value != null && cb){
                  return {
                    "service_uuid":service_uuid,
                    "character_uuid":value,
  ...
  static Future<BluetoothConnection> toAddress(String? address) async {
    ...
    var result = await FlutterBluetoothSerial
        ._methodChannel
    ...
    return BluetoothConnection._consumeConnectionID(id,uuid);
  }
...
class _BluetoothStreamSink<Uint8List> extends StreamSink<Uint8List> {
  ...
  dynamic exception;
   ...
   String? w_service_uuid;
   String? w_character_uuid;

  String? r_service_uuid;
  String? r_character_uuid;
  ...
  void add(Uint8List data) {
    ...
    _chainedFutures = _chainedFutures.then((_) async {
      ...
      await FlutterBluetoothSerial._methodChannel
          .invokeMethod('write', {'id': _id, 'bytes': data,'w_character_uuid':w_character_uuid,'w_service_uuid':w_service_uuid,
        'r_character_uuid':r_character_uuid,'r_service_uuid':r_service_uuid});
    }).catchError((e) {
      this.exception = e;
      close();
    });
